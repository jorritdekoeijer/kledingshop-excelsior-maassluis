...
export async function deleteReorderRulesNotInGarmentTemplates(
  ...
  const youthTpl = garmentType === "socks" ? SOCKS_SIZE_OPTIONS : YOUTH_SIZE_OPTIONS;
  const adultTpl = garmentType === "socks" ? SOCKS_SIZE_OPTIONS : ADULT_SIZE_OPTIONS;
  ...
  for (const r of rules ?? []) {
    const tpl: readonly string[] = r.variant_segment === "youth" ? youthTpl : adultTpl;
