...
import { markOrderFulfilled } from "@/app/dashboard/orders/actions";
...
export default async function DashboardOrdersPage({ searchParams }: Props) {
  ...
  const { data: orders, error } = await supabase
    .from("orders")
    .select("id,status,total_cents,guest_name,guest_email,created_at,fulfillment_error,public_token,confirmation_sent_at")
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-brand-blue">Bestellingen</h1>
        <p className="mt-2 text-sm text-zinc-600">
          Gastorders en betalingen via Mollie. Voorraad wordt na betaling via FIFO afgeboekt. Na levering kun je de order
          op <span className="font-medium">afgehandeld</span> zetten.
        </p>
      </div>
