  ...
  if (!gate.ok) {
    return (
      <div className="rounded-lg border border-zinc-200 bg-white p-6">
        ...
        <p className="mt-2 text-sm text-zinc-600">Je hebt geen permissie om dit gedeelte te bekijken.</p>
      </div>
    );
  }

  const canGeneralSettings =
    isAdmin || perms.includes(permissions.dashboard.access) || perms.includes(permissions.settings.read);
  const canSuppliersCard = hasSuppliersSettingsAccess(perms, { isAdmin });
