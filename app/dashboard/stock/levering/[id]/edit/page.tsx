...
export default async function EditStockDeliveryPage({
  ...
  const byProduct = new Map(options.map((p) => [p.id, p]));
  ...
  const defaults = {
    ...
    lines: (batches ?? []).map((r: any) => {
      const pr = byProduct.get(r.product_id);
      const printing = pr ? Math.max(0, Number((pr as any).printingExclCents ?? 0) || 0) : 0;
      return {
        ...
        // printing wordt bij submit opnieuw berekend in het formulier; hier alleen voor weergave in totalen niet nodig.
        _printing: printing
      };
    })
  };
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-zinc-600">
          Je kunt deze levering bewerken zolang er nog geen voorraad uit is verbruikt. Anders maak je een correctielevering.
        </p>
        {ok ? (
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-6">
        <NewDeliveryForm
          ...
          defaults={{
            ...
            invoiceTotalInclEuro: defaults.invoiceTotalInclEuro,
            lines: (defaults.lines ?? []).map((l: any) => ({
